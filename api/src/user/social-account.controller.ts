...
export class SocialAccountController {
    ...
    @ApiResponse({ status: 409, description: 'Social account already linked to another user' })
    async linkSocialAccount(@Body() linkDto: LinkSocialAccountDto): Promise<SocialAccountEntity> {
    ...
    async getSocialAccounts(@Param('userId') userId: string): Promise<SocialAccountEntity[]> {
    ...
    @ApiResponse({ status: 200, description: 'Social account retrieved successfully' })
    ...
    async getSocialAccountByPlatform(
        ...
        return await this.socialAccountService.getSocialAccountByPlatform(userId, platform);
    }
}