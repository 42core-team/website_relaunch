...
import { Event, events_state_enum, Match, matches_phase_enum, matches_state_enum } from "@/generated/prisma";
...
async function handleSwissRoundState(eventId: string, event: Event, teams: Team[], currentSwissRoundFinished: Match[], maxRounds: number): Promise<boolean> {
...
async function handleEliminationRoundState(eventId: string, event: Event, currentEliminationRoundFinished: Match[]): Promise<boolean> {
...
export async function calcMedianBuchholzPoints(eventId: string): Promise<boolean> {
    ...
    for (const team of teams) {
        const swissMatches = team.matchTeams.map(mt => mt.match).filter(match =>
        ...
        for (const match of swissMatches) {
            const opponent = match.matchTeams.find(mt => mt.team.id !== team.id)?.team;
...
export async function getCurrentPhase(eventId: string): Promise<events_state_enum | null> {
    const event = await prisma.event.findUnique({
        where: { id: eventId },
    });
