...
async function githubApiExample() {
  ...
  try {
    ...
    const repo: GitHubRepository = await repoApi.getRepository(
      "octocat",
      "hello-world",
    );
    ...
    const issues: GitHubIssue[] = await repoApi.listIssues(
      "octocat",
      "hello-world",
      {
        state: "open",
    ...
    const repos: GitHubRepository[] = await userApi.listUserRepositories(
      "octocat",
      {
        sort: "updated",
        direction: "desc",
    ...
    const rateLimit: GitHubRateLimit =
      await githubClient.get<GitHubRateLimit>("rate_limit");
    ...
    if (isCollaborator) {
      ...
      const permissionLevel: GitHubCollaboratorPermission =
        await repoApi.getCollaboratorPermissionLevel(
          myOwner,
          myRepo,
          collaboratorUsername,
        );
      ...
      console.log(
        `Updating permission for ${collaboratorUsername} to 'maintain'...`,
      );
      ...
      console.log(
        `Inviting ${collaboratorUsername} as a collaborator with 'triage' permission...`,
      );
      ...
      invitations.forEach((invitation, index) => {
        console.log(
          `${index + 1}. ${invitation.invitee.login} (ID: ${invitation.id})`,
        );
      });
      console.log("---");
    ...
    myInvitations.forEach((invitation, index) => {
      console.log(
        `${index + 1}. ${invitation.repository.full_name} (ID: ${invitation.id})`,
      );
    });
    console.log("---");
    ...
    const userInvitations =
      await userApi.listRepositoryInvitations(userAccessToken);
    ...
    if (userInvitations.length > 0) {
      ...
      console.log(
        `Accepting invitation with ID: ${invitationId} for the user...`,
      );
    ...
    console.log(
      `Accepting invitation for repository: ${myOwner}/${myRepo} for the user...`,
    );
    const userAccepted = await userApi.acceptRepositoryInvitationByRepo(
      myOwner,
      myRepo,
      userAccessToken,
    );
